...
| 单策略心电图、回撤吓人 | regime 门控风险平价 FOF：最大回撤 **-28.1% → -8.0%**，夏普 **0.81 → 1.06**，卡尔马 **0.75 → 1.10**，月胜率 **53% → 78%** |
| 结果难以审阅 | GuanLan 风格风险仪表盘 + **内嵌实时 Claude 顾问**可质询 |

---

## 前后对比（真实数据，2020-04 → 2026-06，标的池一致）

| 方案 | 年化收益 | 最大回撤 | 夏普 | 卡尔马 | 月胜率 |
|---|---|---|---|---|---|
| 单一动量策略（圣杯陷阱） | 20.23% | **-28.06%** | 0.77 | 0.72 | 53% |
| 等权静态组合（朴素分散） | 7.87% | -9.50% | 0.83 | 0.83 | 51% |
| **regime 门控风险平价 FOF（进取档）** | **10.82%** | **-9.41%** | **1.08** | **1.15** | **78%** |

9 sleeve 的 FOF 把单一策略 -28% 级回撤压到 -9.4%，并**在每个指标上都超越等权基准**（年化 7.87%→10.82%、夏普 0.83→1.08、卡尔马 0.83→1.15、回撤仍更低、月胜率 51%→78%）。**进取档**（用户 2026-06 调校）把单 sleeve 硬上限从 50% 放到 100%，消除「单 survivor 被截断→现金拖累」，年化由 ~8.8% 提到 10.8%——代价是集中度风险上升（详见 [`docs/before-after.md`](docs/before-after.md) 的诚实局限）。若要回到回撤优先的稳健档，`FOFConfig.max_weight=0.5` 即可。这是**修正数据 bug 后**的诚实结果（早期 1.4 卡尔马源于 510300/512890 被截断到 2024+ 平静期）。
