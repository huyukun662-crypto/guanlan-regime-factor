...
### 方式 ② 让 agent 一条龙
说 `跑一遍大势研判` / `现在该进攻还是防御，配什么` → `guanlan-analyst` 按固定步骤走：
研报锚定 → 跑 pipeline → 解读 → 带溯源简报 →（要建议时）`factor-allocation`。
产出每个数字都来自 json、每个机制配 ≥2 条研报引用。**简报末尾它会问「要不要打开仪表盘？」**，
你回「打开」它才会跑 `python scripts/open_dashboard.py`（后台起服务 + 自动开浏览器），不擅自启动。
