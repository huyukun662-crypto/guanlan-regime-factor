...
## 输出格式（研报级回复）
读完 `outputs/factor_allocation.json` 后产一份「配置建议」小节（数字全来自 JSON 字段、标口径）：
1. **姿态结论**：**`{posture}`**（来自大势 `{verdict}`，master_score `{master_score}`/置信 `{confidence}%`）+ `{equity_hint}` 对应建议仓位。
2. **风格倾斜**：`{tilt_mode}`（依据 `tilt_basis`：ic_mean/icir/hit_rate）→ 一句说明为何是动量/反转/中性。
3. **超配 / 低配（表）**：`overweight[]` vs `underweight[]` —

   | 方向 | 风格 | 近 1 月(r_1m) | 类别 |
   |---|---|---|---|
4. **逻辑链**：大势定仓位姿态 → 因子定风格倾斜（一句话串起 2、3）。
5. **诚实口径**：照搬 `caveats[]`；强调**弱倾斜别重押、防御期 tilt 意义小、非组合无权重未回测**；完整读法见 `references/reading-allocation-advice.md`；仅供研究参考。
