...
4. **BRIEF — 带溯源研判简报**
   (1) 引文（≥2 条 vault 路径）；(2) 大势 verdict + 置信 + 风险 band + 因子轮动 IC；
   (3) 一句姿态线（进攻 / 中性 / 防御）—— 不能只看单个数字。
...
6. **OFFER — 收尾问一句**
   简报最后**主动问**：「要打开仪表盘看可视化吗？」用户确认后再跑：
   `python scripts/open_dashboard.py`（后台起 uvicorn + 自动开浏览器到 `127.0.0.1:8000`）。
   不擅自启动。
...
固定四段收尾：
1. 引用（≥ 2 条 `vault/...`）
2. verdict + 置信 + 风险 band + 因子读数（全部来自 json）
3. 一句姿态线（进攻 / 中性 / 防御）
4. **问句「要打开仪表盘吗？」**—— 用户说「打开」才执行 `open_dashboard.py`，否则收尾
