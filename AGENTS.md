...
6. **OFFER — 固定收尾，每次都要：先问仪表盘，最后问用哪个 skill 深入**
   九段正文后给「下一步」收尾，**固定两问、顺序如下**：
   - **(a) 仪表盘**：「要打开仪表盘看可视化吗？（大势研判 + 因子两页 + AI 顾问）」——用户确认
     （「打开 / 好 / open / yes」）后才跑 `python scripts/open_dashboard.py`（后台起 uvicorn + 开浏览器到
     `127.0.0.1:8000`），不擅自启动；已在运行则改问「仪表盘已在 … 运行，要刷新/重开吗？」。
   - **(b)【回复最末一行，固定】**：「要不要我用某个 skill 深入某一块？`regime-radar`(12 风险指标逐项) ·
     `regime-verdict`(HMM 状态/转移细看) · `factor-research`(因子 IC/滚动 Sharpe/相关性) ·
     `factor-allocation`(单出配置建议) · `quant-research-retriever`(就某机制再查研报)。」
     这句 skill 邀请固定是整条回复**最末一行**；用户选了哪个就调对应 skill 深入，说不用就收尾。
...
- **固定收尾（两问，见 step 6）**：倒数第二问「要打开仪表盘吗？」（用户 yes 才执行 `open_dashboard.py`）；
  **回复最末一行固定**是「要不要我用某个 skill 深入某一块？」并列出 5 个 skill——用户选了就调对应 skill 深入。
